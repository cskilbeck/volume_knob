...
props.device.on_control_change = (channel, cc, val) => {
    ...
    // Use the live device config rather than stored_config: stored_config is only
    // populated by on_config_loaded after a flash read response, which can be lost.
    const cfg = props.device.config;
    if (cfg == null || cfg.rot_control_msb === undefined) return;

    let update_knob = false;

    if (channel == get_rot_channel(cfg)) {

        if (is_LSB(cc) && is_rot_extended(cfg) && cc == cfg.rot_control_lsb) {

            if (is_rot_relative(cfg)) {
        ...
        } else if (cc == cfg.rot_control_msb) {
            if ((cfg.flags & midi.flags.cf_rotate_relative) != 0) {
                rotation_value = (rotation_value & 0x3f80) | val;
            } else {
