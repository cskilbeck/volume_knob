...
// report descriptor
__code uint8_t report_descriptor[] = {
...
__code uint8_t DevDesc[18] = {
    ...
    0x00,                    // bcdDevice(1.0) L
    0x01,                    // bcdDevice(1.0) H
    0x01,                    // iManufacturer
    0x02,                    // iProduct
    0x00,                    // iSerialNumber
...
__code uint8_t CfgDesc[41] = {
    ...
    sizeof(report_descriptor) & 0xff,    // wDescriptorLengthL
    sizeof(report_descriptor) >> 8,      // wDescriptorLengthH
    ...
    MAX_PACKET_SIZE & 0xff,    // wMaxPacketSize L
    MAX_PACKET_SIZE >> 8,      // wMaxPacketSize H
    0x02,                      // bInterval

    0x07,                      // bLength
    USB_DESCR_TYP_ENDP,        // bDescriptorType: ENDPOINT
    0x01,                      // bEndpointAddress: OUT/Endpoint1
    0x03,                      // bmAttributes: Interrupt
    MAX_PACKET_SIZE & 0xff,    // wMaxPacketSize L
    MAX_PACKET_SIZE >> 8,      // wMaxPacketSize H
    0x02                       // bInterval
...
void usb_isr(void) __interrupt(INT_NO_USB)
{
    ...
    if(UIF_TRANSFER) {
        ...
        case UIS_TOKEN_OUT | 1:
            UEP1_T_LEN = 0;
            UEP1_CTRL = UEP_R_RES_ACK | UEP_T_RES_ACK;
            break;
...
void usb_set_keystate(uint8_t key)
{
    ...
    *p++ = (uint8_t)0x0;    // modifiers
    *p++ = (uint8_t)0x0;    // pad
    *p++ = (uint8_t)key;    // 6 keys
    *p++ = (uint8_t)0x0;    //
    *p++ = (uint8_t)0x0;    //
    *p++ = (uint8_t)0x0;    //
    *p++ = (uint8_t)0x0;    //
    *p++ = (uint8_t)0x0;    //
    usb_idle = 0;
    UEP1_T_LEN = sizeof(keyboard_report);
